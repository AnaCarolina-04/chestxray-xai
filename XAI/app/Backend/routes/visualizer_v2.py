...
def group_layers_intelligently(layers):
    ...
    - Sequential patterns → "Inverted Residual Block"
    """
    blocks = []
    i = 0
    ...
    while i < len(layers):
        ...
        # Check for Conv Block pattern (Conv2D + BN + Act)
        if layer_type == 'Conv2D' and i + 2 < len(layers):
            next1 = layers[i + 1].get('type', '')
            next2 = layers[i + 2].get('type', '')
            
            if 'BatchNormalization' in next1 and 'Activation' in next2:
                # Create Conv Block
                block = {
                    ...
                    'input_shape': layer.get('shape', 'N/A'),
                    'output_shape': layers[i + 2].get('shape', 'N/A'),
        ...
        if 'DepthwiseConv' in layer_type and i + 2 < len(layers):
            next1 = layers[i + 1].get('type', '')
            next2 = layers[i + 2].get('type', '')
        ...
        if layer_type == 'Dense' and i + 1 < len(layers):
            ...
            if 'Dropout' in next1:
                block = {
                    ...
                    'input_shape': layer.get('shape', 'N/A'),
                    'output_shape': layers[i + 1].get('shape', 'N/A'),
        ...
        block = {
            ...
            'layers': [layer],
            'input_shape': layer.get('shape', 'N/A'),
            'output_shape': layer.get('shape', 'N/A'),
